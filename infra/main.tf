...
module "vpc" {
  ...
  public_subnet_tags  = merge(local.stable_tags, local.public_subnet_tags)
  private_subnet_tags = merge(local.stable_tags, local.private_subnet_tags)

  tags = local.stable_tags
}
...
module "game_eks" {
  ...
  cluster_enabled_log_types = var.enable_cluster_logs ? var.cluster_log_types : []
